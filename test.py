...
# PREP DATA
data = svhn( N, V, batch_size, k=28, transform='none')
data_c = svhn( N, V, batch_size, k, transform='truncated tucker')
...
plt.plot(range(len(acc_train)), acc_train, 'tab:orange', linewidth=3,label = 'svhn-10')
plt.plot(range(len(acc_val)), acc_val, 'tab:orange',linewidth=3, linestyle='--')

plt.plot(range(len(acc_train_c)), acc_train_c, 'tab:green',linewidth=3, label = 'svhn-10 compr.')
plt.plot(range(len(acc_val_c)), acc_val_c, 'tab:green',linewidth=3, linestyle='--')

plt.plot(range(len(acc_train_r)), acc_train_r, 'tab:purple',linewidth=3, label = 'svhn-10 restr.')
...
plt.plot(epsilons, accuracies_c, ".-", label = "svhn-10 compr.")
plt.plot(epsilons, accuracies_r, ".-", label = "svhn-10 restr.")
...
plt.plot(range(len(acc_train_100)), acc_train_100, 'tab:cyan', linewidth=3,label = 'svhn-100')
plt.plot(range(len(acc_val_100)), acc_val_100, 'tab:cyan',linewidth=3, linestyle='--')

plt.plot(range(len(acc_train_c100)), acc_train_c100, 'tab:brown',linewidth=3, label = 'svhn-100 compr.')
plt.plot(range(len(acc_val_c100)), acc_val_c100, 'tab:brown',linewidth=3, linestyle='--')

plt.plot(range(len(acc_train_r100)), acc_train_r100, 'tab:gray',linewidth=3, label = 'svhn-100 restr.')
...
plt.xlabel('epochs')
plt.title(r'Convergence of ResNets on SVHN  ')
plt.savefig('ResNetRankEvol_Accuracy_svhn_compressed.png', bbox_inches='tight')
plt.show()
...
plt.plot(epsilons, accuracies_100, ".-", label = "svhn-100")
plt.plot(epsilons, accuracies_100_c, ".-", label = "svhn-100 compr.")
plt.plot(epsilons, accuracies_100_r, ".-", label = "svhn-100 restr.")
...
plt.legend()
plt.ylabel("Accuracy")
plt.savefig('SVHN_AdversarialAttack_compressions.png', bbox_inches='tight')
plt.show()
